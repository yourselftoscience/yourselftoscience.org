...
const StatsPage = () => {
    ...
    const euOriginBreakdownStats = React.useMemo(() => {
        const euOriginsWithCounts = allResources.reduce((acc, resource) => {
            if (resource.origin) {
        ...
        return Object.entries(euOriginsWithCounts).sort(([, a], [, b]) => b - a);
    }, []);
    ...
    return (
        <main className="flex-grow w-full max-w-screen-xl mx-auto px-4 py-12 md:py-16">
            ...
            >
                ...
                {/* Row 2: Larger Cards */}
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <StatCard title="Resources Based In" icon={<FaMapMarkedAlt size="1.5em" />}>
                        <div className="mt-6">
                            {stats.topOrigins.map(([origin, count], index) => {
                                const maxValue = stats.topOrigins[0][1] || 0;
                                if (origin === 'European Union') {
                                    ...
                                    return (
                                        <div key="eu-origin-expandable">
                                            <div
                                                className="mb-3 cursor-pointer"
                                                title={`European Union: ${count} (click to expand)`}
                                                onClick={() => setIsOriginEuExpanded(!isOriginEuExpanded)}
                                            >
                                                <div className="flex justify-between items-center text-sm mb-1">
                                                    <div className="flex items-center text-apple-primary-text truncate">
                                                        {origin}
                                                        <motion.div animate={{ rotate: isOriginEuExpanded ? 180 : 0 }} className="ml-2 text-apple-accent">
                                                ...
                                                {isOriginEuExpanded && (
                                                    ...
                                                    >
                                                        <div className="pl-4 ml-4 border-l border-apple-divider my-2">
                                                            {euOriginBreakdownStats.map(([euCountry, euCount], euIndex) => (
                                ...
                                return <Bar key={origin} label={origin} value={count} maxValue={maxValue} index={index} />
                            })}
                        </div>
                    </StatCard>
