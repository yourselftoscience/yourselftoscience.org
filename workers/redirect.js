// This worker is completely separate from the Next.js build.
// Its only job is to redirect from an ID to a slug.

let resourceMap = null;

async function getResourceMap() {
  // Simple cache to avoid refetching the JSON on every single request
  if (resourceMap) return resourceMap;

  try {
    const response = await fetch('https://yourselftoscience.org/resources.json');
    if (!response.ok) throw new Error(`Failed to fetch resources: ${response.statusText}`);
    const resources = await response.json();
    const newMap = new Map();
    for (const resource of resources) {
      if (resource.id && resource.slug) {
        newMap.set(resource.id, resource.slug);
      }
    }
    resourceMap = newMap;
    return resourceMap;
  } catch (error) {
    console.error('Error building resource map:', error);
    return new Map();
  }
}

export default {
  async fetch(request) {
    const { pathname } = new URL(request.url);
    const pathParts = pathname.split('/').filter(Boolean);

    // Check for the specific pattern: /resource/<uuid>
    if (pathParts.length === 2 && pathParts[0] === 'resource') {
      const uuid = pathParts[1];
      const map = await getResourceMap();
      const slug = map.get(uuid);

      if (slug) {
        // If a slug is found, redirect to the correct page on the main site.
        const destinationURL = `https://yourselftoscience.org/resource/${slug}`;
        return Response.redirect(destinationURL, 308); // 308 is for permanent redirects
      }
    }

    // For any other path on id.yourselftoscience.org, go to the main site's homepage.
    // This solves the duplicate content SEO issue.
    return Response.redirect('https://yourselftoscience.org', 301);
  },
};